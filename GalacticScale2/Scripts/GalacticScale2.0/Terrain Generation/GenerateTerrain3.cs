    ...
    public static partial class TerrainAlgorithms
    {
        public static void GenerateTerrain3(GSPlanet gsPlanet, double modX = 0.0, double modY = 0.0)
        {
            ...
            for (var index = 0; index < data.dataLength; ++index)
            {
                ...
                var f = num10 * 2.0 + 0.92 +
                        Mathf.Clamp01((float) (num11 * Mathf.Abs((float) num13 + 0.5f) - 0.35) * 1f);
                //GS2.Log(f.ToString());
                if (f < 0.0) f *= 2.0;
                ...
                var num15 = num14 <= 0.0 ? Mathf.Lerp(-4f, 0.0f, (float) num14 + 1f) :
                    num14 <= 1.0 ? Mathf.Lerp(0.0f, 0.3f, (float) num14) + num12 * 0.1 :
                    num14 <= 2.0 ? Mathf.Lerp(0.3f, 1.4f, (float) num14 - 1f) + num12 * 0.12 :
                ...
                data.heightData[index] =
                    (ushort) ((planet.radius + num15 * t.HeightMulti + 0.2 + t.BaseHeight) * 100.0);
                data.biomoData[index] =
                    (byte) Mathf.Clamp((float) (num19 * 100.0 * t.BiomeHeightMulti + t.BiomeHeightModifier), 0.0f,
                        200f);
            }
            //GS2.Log("--------------------");
        }
