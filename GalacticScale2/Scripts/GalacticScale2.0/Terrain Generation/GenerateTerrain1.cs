    ...
    public static partial class TerrainAlgorithms
    {
        public static void GenerateTerrain1(GSPlanet gsPlanet, double modX = 0.0, double modY = 0.0)
        {
            ...
            for (var i = 0; i < data.dataLength; ++i)
            {
                ...
                var noise1 =
                    simplexNoise1.Noise3DFBM(x * (t.xFactor + 0.01), y * (0.012 + t.yFactor), z * (0.01 + t.zFactor),
                        6) * 3 * t.HeightMulti + (-0.2 + t.BaseHeight);
                var noise2 =
                    simplexNoise2.Noise3DFBM(x * (1.0 / 400.0), y * (1.0 / 400.0), z * (1.0 / 400.0), 3) * 3 *
                    t.HeightMulti * (t.RandomFactor + 0.9) + (t.LandModifier + 0.5);
                ...
                var noise3 = simplexNoise2.Noise3DFBM(x * (t.xFactor + 0.01) * 2.5, y * (0.012 + t.yFactor) * 8.0,
                    z * (0.01 + t.zFactor) * 2.5, 2) * 0.6 - 0.3;
