    ...
    public static partial class TerrainAlgorithms
    {
        ...
        public static void GenerateTerrain5(GSPlanet gsPlanet, double modX = 0.0, double modY = 0.0)
        {
            ...
            for (var i = 0; i < data.dataLength; i++)
            {
                ...
                data.heightData[i] =
                    (ushort) ((gsPlanet.planetData.radius + hdEnd + 0.2 + t.BaseHeight) * 100.0 * t.HeightMulti);
                data.biomoData[i] =
                    (byte) Mathf.Clamp((float) ((bdEnd + t.BiomeHeightModifier) * 100.0 * t.BiomeHeightMulti), 0f,
                        200f);
            }
        }
    }
