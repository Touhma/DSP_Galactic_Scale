    ...
    public class PatchOnPlayerMove_Fly
    {
	    ...
	    public static bool GameTick(ref PlayerMove_Fly __instance)
	    {
		    ...
		    if (__instance.player.movementState == EMovementState.Fly)
		    {
			    ...
			    float num8 = Mathf.Clamp(num7 * 0.5f, -10f, 10f) * 100f + (float)__instance.controller.universalGravity.magnitude;
			    ...
			    float num9 = (float)(Math.Sin(GlobalObject.timeSinceStart * 2.0) * 0.1 + 1.0);
			    if (Mathf.Abs(__instance.verticalThrusterForce) > 0.001f)
			    {
				    __instance.controller.AddLocalForce(normalized * (__instance.verticalThrusterForce * num9));
			    }
			    ...
			    vector4 -= b;
			    float num10 = __instance.controller.vertSpeed;
			    float num11 = Mathf.Lerp(0.95f, 0.8f, Mathf.Abs(num7) * 0.3f);
			    float num12 = num11;
