...
namespace GalacticScale
{
    public class PatchOnBuildTool_Path
    {
            ...
            ref bool ___cursorValid)
        {
            ...
            var preview2 = __instance.buildPreviews[__instance.buildPreviews.Count - 1];
            var objId = preview.inputObjId;
            var objId2 = preview2.outputObjId;
            //GS2.Warn(objId.ToString());
            var ___factory = __instance.factory;
            if (objId < 0 && objId2 < 0 ||
                objId >= ___factory.entityPool.Length && objId2 >= ___factory.entityPool.Length)
                return __result; // Sanity Check

            var entity = ___factory.entityPool[objId];
            var entity2 = ___factory.entityPool[objId2];
            ...
            if (itemProto != null && itemProto.prefabDesc.oilMiner ||
                itemProto != null && itemProto.prefabDesc.veinMiner ||
                itemProto != null && itemProto.prefabDesc.isStation ||
                itemProto2 != null && itemProto2.prefabDesc.isStation) // Check that we are connected to an oil miner
                //GS2.Log($"{preview?.condition} | {preview2?.condition}");
                if (preview?.condition == EBuildCondition.JointCannotLift ||
                    preview2?.condition ==
                    EBuildCondition.JointCannotLift) // Make sure the error is that the endpoint must be horizontal
                {
                    if (preview?.condition == EBuildCondition.JointCannotLift)
