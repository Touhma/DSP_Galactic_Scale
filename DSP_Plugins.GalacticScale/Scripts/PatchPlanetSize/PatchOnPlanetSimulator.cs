...
namespace GalacticScale.Scripts.PatchPlanetSize {
    ...
    public class PatchOnPlanetSimulator {
        ...
        public static bool UpdateUniversalPosition(ref PlanetSimulator __instance, ref StarSimulator ___star, ref bool ___isLocal, ref Transform ___lookCamera, Vector3 playerLPos, VectorLF3 playerUPos, Vector3 cameraPos) {
            if (__instance.planetData == null || __instance.planetData.loading || __instance.planetData.factoryLoading) return false;
            ...
            if (__instance.planetData == localPlanet) {
                if (__instance.transform.localPosition.sqrMagnitude > 0.0) __instance.transform.localPosition = Vector3.zero;
                if (__instance.transform.localRotation != Quaternion.identity) __instance.transform.localRotation = Quaternion.identity;
            }
            else {
