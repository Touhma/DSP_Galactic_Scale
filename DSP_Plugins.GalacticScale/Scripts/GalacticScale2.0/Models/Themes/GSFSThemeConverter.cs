...
namespace GalacticScale {
    ...
    public class GSFSThemeConverter : fsDirectConverter<GSTheme> {
        public override object CreateInstance(fsData data, Type storageType) => new GSTheme();
        protected override fsResult DoSerialize(GSTheme model, Dictionary<string, fsData> serialized) {
            ...
            if (!GS2.minifyJSON) {
                ...
                if (!based || model.Algo != baseTheme.Algo) {
                ...
                if ((!based || model.GasItems != baseTheme.GasItems) && model.GasItems != null && model.GasItems.Length > 0) {
                    SerializeMember(serialized, null, "GasItems", model.GasItems);
                }

                if ((!based || model.GasSpeeds != baseTheme.GasSpeeds) && model.GasSpeeds != null && model.GasSpeeds.Length > 0) {
                    SerializeMember(serialized, null, "GasSpeeds", model.GasSpeeds);
                }

                if (!based || model.Wind != baseTheme.Wind) {
                ...
                if ((!based || !Utils.ArrayCompare(model.Musics, baseTheme.Musics)) && model.Musics != null && model.Musics.Length > 0) {
                ...
                if ((!based || model.atmosphereMaterial != baseTheme.atmosphereMaterial) && model.atmosphereMaterial != null) {
        ...
        protected override fsResult DoDeserialize(Dictionary<string, fsData> data, ref GSTheme model) {
            ...
            if (model.baseTheme != null) {
                model.CopyFrom(model.baseTheme);
            }
            ...
            if (data.ContainsKey("PlanetType")) {
            ...
            if (data.ContainsKey("IonHeight")) {
                DeserializeMember(data, null, "IonHeight", out model.IonHeight);
            }

            if (data.ContainsKey("WaterHeight")) {
                DeserializeMember(data, null, "WaterHeight", out model.WaterHeight);
            }

            if (data.ContainsKey("WaterItemId")) {
                DeserializeMember(data, null, "WaterItemId", out model.WaterItemId);
            }

            if (data.ContainsKey("Musics")) {
                DeserializeMember(data, null, "Musics", out model.Musics);
            }

            if (data.ContainsKey("SFXPath")) {
            ...
            if (data.ContainsKey("MaterialPath")) {
