...
namespace GalacticScale
{
    public static class HandleLocalStarPlanets
    {
        ...
        static StarData closestStar;
        static PlanetData closestPlanet;
        static GSPlanet gsPlanet;
        static Dictionary<PlanetData, double> transitionRadii = new Dictionary<PlanetData, double>();
        ...
        private static int GetLoadedPlanetCount(StarData star)
        {
            ...
            {
                if (star.planets[i].loaded) planetsLoaded++;
            }
        ...
        private static void SearchStar()
        {
            for (int i = 0; closestStar == null && i < GameMain.galaxy.starCount; i++)
            {
                ...
                if (DistanceTo(star) < TransisionDistance(star)) closestStar = star;
                if (!star.loaded && GameMain.isRunning) star.Load();
            }
        }
        ...
        private static double TransisionDistance(PlanetData planet)
        {
            ...
            if (gsPlanet.MoonCount > 0)
            {
                ...
                else GS2.Warn($"Not setting anything, as td:{transitionDistance} < d:{distance}");
            }
