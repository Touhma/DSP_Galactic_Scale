...
namespace GalacticScale.Generators
{
    public partial class GS2Generator : iConfigurableGenerator
    {
        ...
        public void SetGalaxyDensity(int density)
        {
            switch (density)
            {
                case 1: minStepLength = 1.2f; maxStepLength = 1.5f; minDistance = 1.2f; break;
                case 2: minStepLength = 1.4f; maxStepLength = 2f; minDistance = 1.5f; break;
                case 3: minStepLength = 1.6f; maxStepLength = 2.5f; minDistance = 1.7f; break;
                case 4: minStepLength = 1.8f; maxStepLength = 3f; minDistance = 2f; break;
                case 5: minStepLength = 2f; maxStepLength = 3.5f; minDistance = 2.3f; break;
                case 6: minStepLength = 2.2f; maxStepLength = 4.2f; minDistance = 2.4f; break;
                case 7: minStepLength = 2.5f; maxStepLength = 5.0f; minDistance = 2.6f; break;
                case 8: minStepLength = 2.7f; maxStepLength = 6.0f; minDistance = 2.8f; break;
                case 9: minStepLength = 3.0f; maxStepLength = 7.0f; minDistance = 3.0f; break;
        ...
        private void GeneratePlanetsForStar(GSStar star)
        {
            ...
            for (var i=1;i<starBodyCount;i++)
            {
                ...
                {
                    ProtoPlanet p = new ProtoPlanet() { gas = CalculateIsGas(star), radius = GetStarPlanetSize(star) };
            ...
            {
                if (protos[i].gas && protos[i].radius > 80) {
                    //GS2.Warn("RADIUS 80"); 
                    protos[i].radius = 80; 
                }
            ...
            foreach (ProtoPlanet proto in protos) {
                if (proto.gas)
                {
                    if (proto.radius < 50) Warn("GAS AND NOT 50");
        ...
        private void FudgeNumbersForPlanets(GSStar star)
        {
            foreach (GSPlanet body in star.Bodies)
            {
                ...
                if (!IsPlanetOfStar(star, body)) body.OrbitInclination = (random.NextBool()?1:-1) * (10f + random.NextFloat() * 50f);
            ...
            for (var planetIndex = 0; planetIndex < star.PlanetCount; planetIndex++)
            {
                ...
                for (var moonIndex = 0; moonIndex < planet.MoonCount; moonIndex++)
                {
                    ...
                    for (var moon2Index = 0; moon2Index < moon.MoonCount; moon2Index++)
                    {
                        ...
                        moon2.Name = $"{star.Name} - {roman[planetIndex+1]} - {roman[moonIndex +1]} - {roman[moon2Index +1]}";
        ...
        private void PickNewBirthPlanet()
        {
            ...
            {
                Warn("Generating new habitable planet by overwriting an existing one");
                ...
                Warn($"Setting Planet Theme to {themeName}");
                planet.Theme = themeName;
                Warn("Setting birthPlanet");
                birthPlanet = planet;
                birthPlanetIndex = index;
                birthStar = star;
                Warn($"Selected {birthPlanet.Name}");
                LogJson(planet, true);
                return;
            }
            else if (HabitablePlanets.Count > 1)
            {
                Warn("Selecting random habitable planet");
                ...
                for (var i = 0; i< birthStar.PlanetCount; i++)
                {
                    if (birthStar.Planets[i] == birthPlanet)
                    {
                        ...
                        birthPlanetIndex = i;
                        Warn($"Selected {birthPlanet.Name} as birthPlanet (planet) index {i} of star {birthStar.Name}");
                    }
                    for (var j=0;j < birthStar.Planets[i].Moons.Count; j++)
                    {
                        if (birthStar.Planets[i].Moons[j] == birthPlanet)
                        {
                            ...
                            birthPlanetIndex = j;
                            Warn($"Selected {birthPlanet.Name} as birthPlanet (moon) index {j} of planet {birthPlanetHost.Name} ");
                            return;
                        }
