...
namespace GalacticScale.Generators
{
    public partial class Sol : iConfigurableGenerator
    {
        ...
        public GSPlanet RandomMoon(GSStar star, GSPlanet host, string name, int index, int orbitCount, string heat)
        {
            ...
            int radius = random.Next(30, Mathf.Clamp(host.Radius - 10,30,510));
        ...
        {
            int radius = 200;
            int maxPlanetSize = preferences.GetInt("maxPlanetSize", 300);
            int minPlanetSize = preferences.GetInt("minPlanetSize", 30);
            int avgPlanetSize = ((maxPlanetSize - minPlanetSize) / 2) + minPlanetSize;
            if (!tiny && !huge)
            { //average
