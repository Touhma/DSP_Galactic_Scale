...
namespace GalacticScale
{
    public class PatchOnPlayerMove_Sail
    {
        ...
        {
            __instance.maxSailSpeed = __instance.player.mecha.maxSailSpeed * 2;
            __instance.maxWarpSpeed = __instance.player.mecha.maxWarpSpeed * 10;
            double dt = 1.0 / 60.0;
            if (__instance.player.sailing)
            {
                ...
                float num1 = !guideRunning ? 1f : 0.4f;
                __instance.energy_scale = !guideRunning ? 1.0 : 0.0;
                if (__instance.player.mecha.thrusterLevel >= 3)
                {
                    ...
                    if (__instance.player.warping && __instance.player.warpCommand && localStar != null)
                    {
                        for (int index = 0; index < localStar.planetCount; ++index)
                        {
                            ...
                            double num2 = VectorLF3.Dot(normalized1, normalized2);
                            if (magnitude < (double)planet.realRadius + 950.0 && num2 > Math.Sqrt(Math.Max(0.0, magnitude * magnitude - (double)planet.realRadius * (double)planet.realRadius * 1.5)) / magnitude)
                            {
                                __instance.player.warpCommand = false;
                                VFAudio.Create("warp-end", __instance.player.transform, Vector3.zero, true);
                ...
                if (localPlanet != null)
                {
                    ...
                    num3 = v.magnitude - (double)localPlanet.realRadius;
                    ...
                    num4 = Maths.Clamp01((600.0 - num3) / 450.0);
                    num5 = Maths.Clamp01((600.0 - num3) / 300.0);
                    vectorLf3_2 = vectorLf3_1 * num4;
                }
                ...
                {
                    bool flag4 = (double)vector3_1.y > 0.5;
                    if (!flag4 && (double)vector3_1.x * (double)vector3_1.x + (double)vector3_1.z * (double)vector3_1.z > 0.25)
                        vector3_1.y += 1.2f;
                    ...
                    VectorLF3 normalized = (vectorLf3_3 * (double)vector3_1.y + vectorLf3_4 * (double)vector3_1.z + vectorLf3_5 * (double)vector3_1.x).normalized;
                    double num2 = magnitude1;
                    if (flag4)
                    {
                        flag3 = true;
                        num2 = magnitude1 <= 100.0 ? 100.0 : magnitude1;
                        if (flag2 && !__instance.player.warping)
                        {
                            ...
                            double num7 = dvel * __instance.UseSailEnergy(dvel);
                            num2 += num7;
                        }
                    }
                    VectorLF3 vectorLf3_6 = normalized * num2 + vectorLf3_2;
                ...
                if (!flag3 && flag2 && !__instance.player.warping)
                {
                    ...
                    VectorLF3 vectorLf3_3 = __instance.visual_uvel.normalized * num8 + vectorLf3_2;
                ...
                if (localPlanet != null)
                {
                    ...
                    double num2 = num4 * num4 * 0.08 * (1.0 - __instance.input_aff_1) * (1.0 - __instance.input_aff_1);
                    double num7 = vectorLf3_5.x * vectorLf3_4.x + vectorLf3_5.y * vectorLf3_4.y + vectorLf3_5.z * vectorLf3_4.z;
                    VectorLF3 vectorLf3_6 = vectorLf3_4 * num7;
                    VectorLF3 vectorLf3_7 = vectorLf3_5 - vectorLf3_6;
                    double magnitude4 = vectorLf3_7.magnitude;
                    double num8 = magnitude4 * magnitude4 / magnitude3 * dt;
                    double num9 = Math.Sqrt(magnitude2 * magnitude3 * 0.7);
                    double num10 = magnitude4 * (1.0 - num2) + num9 * num2;
                    double num11 = num3 <= 60.0 ? (60.0 - num3) * 0.5 : -(num3 - 60.0) * 0.01;
                    double num12 = num7 * (1.0 - num2) + num11 * num2 - num8 * num5 * (1.0 - __instance.input_aff_1);
                    ...
                    double num13 = num4 * (1.0 - __instance.input_aff_2);
                ...
                else if (localStar != null && !__instance.player.warping)
                {
                    ...
                    if (num2 < 1000.0)
                    {
                        num6 = Maths.Clamp01((1000.0 - num2) / 500.0);
                        VectorLF3 uVelocity = __instance.player.uVelocity;
                        double num7 = num6 * num6 * 0.08 * (1.0 - __instance.input_aff_1) * (1.0 - __instance.input_aff_1);
                        double num8 = uVelocity.x * vectorLf3_4.x + uVelocity.y * vectorLf3_4.y + uVelocity.z * vectorLf3_4.z;
                        VectorLF3 vectorLf3_5 = vectorLf3_4 * num8;
                        VectorLF3 vectorLf3_6 = uVelocity - vectorLf3_5;
                        double magnitude4 = vectorLf3_6.magnitude;
                        double num9 = magnitude4 * magnitude4 / magnitude3 * dt;
                        double num10 = Math.Sqrt(magnitude2 * magnitude3 * 2.0);
                        double num11 = magnitude4 * (1.0 - num7) + num10 * num7;
                        double num12 = num2 <= 300.0 ? (300.0 - num2) * 0.1 : -(num2 - 300.0) * 0.01;
                        double num13 = num8 * (1.0 - num7) + num12 * num7 - num9 * num6 * (1.0 - __instance.input_aff_1);
                        VectorLF3 dvel = vectorLf3_4 * num13 + vectorLf3_6.normalized * num11 - __instance.player.uVelocity;
                        __instance.UseSailEnergy(ref dvel, 1.5);
                        if (!disableController && !guideRunning)
                            __instance.player.uVelocity += dvel;
                        double num14 = num6 * (1.0 - __instance.input_aff_2);
                ...
                if (localPlanet != null && num5 > 0.0 && !__instance.player.warping)
                {
                    ...
                    Quaternion quaternion = Quaternion.Slerp(Quaternion.identity, Quaternion.FromToRotation((Vector3)normalized, (Vector3)vectorLf3_5), (float)num5);
                    if (!disableController)
                    {
                ...
                else if (localStar != null && !__instance.player.warping && num6 > 0.0)
                {
                    VectorLF3 vectorLf3_3 = __instance.player.uPosition - localStar.uPosition;
                    ...
                    Quaternion quaternion = Quaternion.Slerp(Quaternion.identity, Quaternion.FromToRotation((Vector3)vectorLf3_3, (Vector3)vectorLf3_4), (float)num6);
                    if (!disableController)
                    {
                        __instance.sailPoser.targetURot = quaternion * __instance.sailPoser.targetURot;
                        __instance.sailPoser.targetURotWanted = quaternion * __instance.sailPoser.targetURotWanted;
                ...
                if (localPlanet != null && !__instance.player.warping && !guideRunning)
                {
                    ...
                    {
                        if (num3 < 46.0 && magnitude5 < 75.0 && (double)num2 < 7.0 || num3 < 25.0 && magnitude5 < 85.0 && (double)num2 < 18.0 || num3 < 7.0)
                        {
                            __instance.controller.movementStateInFrame = EMovementState.Fly;
                            __instance.controller.actionFly.targetAltitude = (float)num3/2;
                    ...
                    { //////////Added 4.8  instead of 48
                        __instance.SoftLimit(localPlanet.uPosition, (double)localPlanet.realRadius, (double)localPlanet.realRadius + (48), 1f, ref __instance.player.uPosition, ref __instance.player.uVelocity);
                        //GS2.Log(num3.ToString());
                        if (num3 < (48))
                        {
                            
                            __instance.controller.movementStateInFrame = EMovementState.Fly;
                            __instance.controller.actionFly.targetAltitude = Mathf.Max(2f, (float)num3);
                            GameCamera.instance.SyncForSailMode();
                            //if (!guideRunning)
                            //    __instance.controller.velocityOnLanding = Maths.QInvRotate(q, (Vector3)__instance.visual_uvel);
                            //else
