    ...
    public partial class PatchOnUIVirtualStarmap {
        ...
        public static bool OnGalaxyDataReset(ref UIVirtualStarmap __instance)
        {
            ...
            foreach (UIVirtualStarmap.StarNode starNode in __instance.starPool)
            {
                ...
                starNode.starData = (StarData)null;
            ...
            foreach (UIVirtualStarmap.ConnNode connNode in __instance.connPool)
            {
                ...
                connNode.starA = (StarData)null;
                connNode.starB = (StarData)null;
                connNode.lineRenderer.gameObject.SetActive(false);
            }
            if (__instance._galaxyData == null)
                return false;
            int index1 = 0;
            for (int index2 = 0; index2 < __instance._galaxyData.stars.Length; ++index2)
            {
                while (__instance.starPool.Count <= index2)
                    __instance.starPool.Add(new UIVirtualStarmap.StarNode()
                    {
                        ...
                        pointRenderer = Object.Instantiate<MeshRenderer>(__instance.starPointPrefab, __instance.starPointPrefab.transform.parent),
                        nameText = Object.Instantiate<Text>(__instance.nameTextPrefab, __instance.nameTextPrefab.transform.parent)
                    });
                StarData star1 = __instance._galaxyData.stars[index2];
                Color a1 = __instance.starColors.Evaluate(star1.color);
                ...
                if (star1.index == GSSettings.birthStarId)
                    str = "即将登陆".Translate() + "\r\n" + str;
                ...
                __instance.starPool[index2].pointRenderer.transform.localPosition = (Vector3)star1.position;
                ...
                StarGraphNode graphNode = __instance._galaxyData.graphNodes[index2];
                for (int index3 = 0; index3 < graphNode.lines.Count; ++index3)
                {
                    StarGraphNode line = graphNode.lines[index3];
                    if (line.index > graphNode.index)
                    {
                        while (__instance.connPool.Count <= index1)
                            __instance.connPool.Add(new UIVirtualStarmap.ConnNode()
                            {
                                ...
                                starA = (StarData)null,
                                starB = (StarData)null,
                                lineRenderer = Object.Instantiate<LineRenderer>(__instance.connLinePrefab, __instance.connLinePrefab.transform.parent)
                            });
                        StarData star2 = __instance._galaxyData.stars[line.index];
                        Color a2 = __instance.starColors.Evaluate(star2.color);
                        ...
                        else if (star2.type == EStarType.BlackHole)
                            a2 = __instance.blackholeColor;
                        float num2 = 1.2f;
                        ...
                        float num3 = Vector3.Distance((Vector3)star1.position, (Vector3)star2.position);
                        float t1 = (float)((double)num1 * 0.25 + 0.349999994039536) / num3;
                        float t2 = (float)(1.0 - ((double)num2 * 0.25 + 0.349999994039536) / (double)num3);
                        __instance.connPool[index1].lineRenderer.SetPosition(0, Vector3.Lerp((Vector3)star1.position, (Vector3)star2.position, t1));
                        __instance.connPool[index1].lineRenderer.SetPosition(1, Vector3.Lerp((Vector3)star1.position, (Vector3)star2.position, t2));
