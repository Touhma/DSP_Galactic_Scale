        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (var i = 0; i < codes.Count; i++)
                if (codes[i].opcode == OpCodes.Ldc_I4_S && codes[i].OperandIs(10))
                {
                    ...
                    newInstructions.Add(
                        new CodeInstruction(
                            codes[i - 1])); //The line before adding 10 is the line which loads in the planet ID we care about, so copy it
                    ...
                    codes.InsertRange(i,
                        newInstructions); //Instead, load the count of planets around the target star (plus one)
                }
