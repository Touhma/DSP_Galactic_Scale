...
namespace GalacticScale.Generators
{
    public partial class GS3Generator : iConfigurableGenerator
    {
        ...
        private void AssignPlanetOrbits(GSStar star)
        {
            // GS3.Warn("-------------------------------------------------------------------------------");
            // GS3.Warn($"--{star.Name}-----------------------------------------------------------------------------");
            // GS3.Warn($"--{star.displayType}-----------------------------------------------------------------------------");
            // GS3.Warn($"Assigning Planet Orbits for {star.Name}:{star.Planets.Count} planets to assign");
            orbitSpacing = preferences.GetFloat("orbitSpacing", 0.05f);
            var r = new GS3.Random(star.Seed);
            ...
            // GS3.WarnJson((from s in planets select s.details).ToList());
            CalculateHabitableZone(star);
            var minimumOrbit = CalculateMinimumOrbit(star);
            var maximumOrbit = CalculateMaximumOrbit(star);
            // GS3.Warn($"Minimum Orbit:{minimumOrbit} Maximum Orbit:{maximumOrbit} Orbit Spacing:{orbitSpacing}");
            ...
            if (star == birthStar)
            {
                ...
                {
                    GS3.Error("Failed to find Actual BirthPlanet or Host");
                }
                // else GS3.Log($"Found Actual BirthPlanet or Host = {actualBirthPlanetOrHost?.Name}");
            }
            //Warn("Orbit Count 0");
            // GS3.Log($"BirthStar = {birthStar.Name} {(birthPlanet != null ? birthPlanet.Name : "null")}");
            if (star == birthStar)
            {
                ...
                // GS3.Warn($"Selected Orbit {birthRadius} for planet {birthPlanet.Name}. Hz:{star.genData.Get("minHZ").Float(0f)}-{star.genData.Get("maxHZ").Float(100f)} StarRadiusAU:{star.RadiusAU}");
            ...
            // GS3.Warn($"Begin Loop:{star.Planets.Count} Planets to fit between {minimumOrbit} and {maximumOrbit}");
            for (var i = 0; i < planets.Count; i++)
            {
                Orbit orbit;
                var planet = planets[i];
                // GS3.Log($"Finding Orbit for planet index {i} - {planet.Name}");
                ...
                // GS3.Log($"Orbit Count > 1. Free orbit range count = {freeOrbitRanges.Count}");
                var availableOrbits = new List<(float inner, float outer)>();
                foreach (var range in freeOrbitRanges)
                {
                    // GS3.Log($"Free orbits:{range}. Checking SystemRadius:{planet.SystemRadius}. {0.05f + 2 * planet.SystemRadius}");
                    ...
                    if (range.outer - range.inner >
                        ...
                        // GS3.Warn($"Adding {range} {preferences.GetFloat("orbitSpacing", 0.05f)} {range.outer - range.inner} {preferences.GetFloat("orbitSpacing", 0.05f) + 2 * planet.SystemRadius}");
                        availableOrbits.Add(range);
                }

                if (availableOrbits.Count == 0)
                {
                    // GS3.Warn("Free Orbit Ranges:");
                    // GS3.LogJson(freeOrbitRanges);
                    // GS3.Warn($"No Orbit Ranges found for planet {planet.Name} {planet.genData["hosttype"]} {planet.genData["hostname"]} radius:{planet.SystemRadius}");
                    var success = false;
                    foreach (var existingOrbit in orbits)
                        if (existingOrbit.hasRoom && existingOrbit.SystemRadius > planet.SystemRadius)
                        {
                            //GS3.Warn($"Existing orbit {existingOrbit.radius} used for planet {planet.Name}");
                    ...
                    //GS3.Log($"{planet.Name} orbit radius {planet.OrbitRadius}");
                    if (success) continue;

                    GS3.Warn($"Couldn't find an orbit for Planet {planet.Name} SysRadius:{planet.SystemRadius} HostType:{planet.genData["hosttype"]} HostName:{planet.genData["hostname"]} . Throwing planet into the sun.");
                ...
                {
                    GS3.Log($"No Available Orbits Found for Planet {planet.Name}");
                    continue;
                }
                ...
                // GS3.Log($"radius = r.NextFloat({selectedRange.inner + planet.SystemRadius}, {selectedRange.outer - planet.SystemRadius})");
                ...
                // GS3.Log($"-{planet.Name} orbit radius {planet.OrbitRadius} from {rangeMin}, {rangeMax}");

                planet.OrbitalPeriod = Utils.CalculateOrbitPeriod(planet.OrbitRadius);
                // GS3.Warn($"selected orbit({radius}) for {planet.Name}({planet.SystemRadius}) SelectedRange:{selectedRange.inner}, {selectedRange.outer} New Ranges: {selectedRange.inner},{radius - planet.SystemRadius}({radius - planet.SystemRadius - selectedRange.inner}) | {radius + planet.SystemRadius}, {selectedRange.outer}({selectedRange.outer - radius - planet.SystemRadius})");
                orbits.Add(orbit);
            ...
            {
                GS3.Warn($"Removing Planet {brokenPlanet}");
                star.Planets.Remove(brokenPlanet);
            }
            //
            // // GS3.Warn($"***** Free orbit count = {freeOrbitRanges.Count}");
            ...
            //     {
            //         // GS3.Log($"Free Orbit Range:{f.inner}:{f.outer}");
            ...
            // // GS3.Log("Possible Hive Orbits:");
            // // GS3.LogJson(possibleHiveOrbits);
            //
            ...
            // GS3.Log($"Planets assigned {(birthPlanet != null ? birthPlanet.Name : "null")}");
        }
        ...
        private void SetPlanetOrbitPhase()
        {
            ...
            var r = new GS3.Random(GSSettings.Seed);
        ...
        public class Orbit
        {
            ...
            public bool hasRoom
            {
                get
                {
                    ...
                    // GS3.Log($"HasRoom Circumference = {circumference} largestRadius = {largestRadius} Planet Count = {planets.Count}");
