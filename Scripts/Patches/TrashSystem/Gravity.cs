...
namespace GalacticScale
{
    internal class PatchOnTrashSystem
    {
        ...
        {
            instructions = new CodeMatcher(instructions).MatchForward(true, new CodeMatch(OpCodes.Add), new CodeMatch(OpCodes.Stloc_2), new CodeMatch(OpCodes.Ldloc_S), new CodeMatch(OpCodes.Ldc_I4_1), new CodeMatch(OpCodes.Add), new CodeMatch(OpCodes.Stloc_S), new CodeMatch(OpCodes.Ldloc_S), new CodeMatch(OpCodes.Ldloc_1), new CodeMatch(OpCodes.Ldc_I4_8)).SetAndAdvance(OpCodes.Ldarg_S, 6).Insert(Transpilers.EmitDelegate<Func<int, int>>(localPlanetId =>
            {
