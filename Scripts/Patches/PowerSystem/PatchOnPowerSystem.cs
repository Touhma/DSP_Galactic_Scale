...
namespace GalacticScale
{
    public class PatchOnPowerSystem
    {
        ...
        public static bool NewGeneratorComponent(int entityId, PrefabDesc desc, ref int __result, PowerSystem __instance)
        {
            ...
            {
                var array = __instance.genRecycle;
                var num = __instance.genRecycleCursor - 1;
                __instance.genRecycleCursor = num;
                num2 = array[num];
            }
            else
            {
                var num = __instance.genCursor;
                __instance.genCursor = num + 1;
                num2 = num;
                if (num2 == __instance.genCapacity) __instance.SetGeneratorCapacity(__instance.genCapacity * 2);
            }
            ...
            var num3 = __instance.planet.realRadius + __instance.planet.ionHeight * 0.6f;
            ...
            var pos = __instance.factory.entityPool[entityId].pos;
            var rot = __instance.factory.entityPool[entityId].rot;
            if (desc.geothermal)
            {
                //GS2.Warn($"GEO RealRadius:{__instance.planet.realRadius } QModHeight:{__instance.planet.data.QueryModifiedHeight(pos)}");
                var num4 = __instance.planet.realRadius - 4f - __instance.planet.data.QueryModifiedHeight(pos);
                var num5 = Mathf.Clamp(num4 * (num4 > 0f ? 0.3f : 0.9f), -0.9f, 0.3f) * 0.05f;
                for (var i = 0; i < PowerSystem.gthDetectionPos.Length; i++)
                {
                    ...
                    if (num4 > 0f && i > 0)
                    {
                        var num6 = __instance.planet.realRadius - 4f - __instance.planet.data.QueryModifiedHeight(pos + rot * (PowerSystem.gthDetectionPos[i] * 1.2f));
                        var num7 = num6 > 0f ? 0.4f : 0.6f;
        ...
        public static bool CalculateGeothermalStrenth(Vector3 pos, Quaternion rot, ref float __result, PowerSystem __instance)
        {
            if (GS2.Vanilla) return true;
            var num = 0f;
            var rr = __instance.planet.realRadius;
            ...
            if (__instance.planet.waterItemId == -1)
            {
                for (var i = 0; i < PowerSystem.gthDetectionPos.Length; i++)
                {
                    var num2 = __instance.planet.realRadius - 4f - __instance.planet.data.QueryModifiedHeight(pos);
                    if (i < 2)
                    {
            ...
            else if (__instance.planet.orbitAroundPlanet.orbitAroundPlanet.orbitAroundPlanet == null) modifier -= __instance.planet.orbitAroundPlanet.orbitAroundPlanet.orbitRadius;
