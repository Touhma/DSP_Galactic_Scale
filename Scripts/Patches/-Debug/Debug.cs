...
// }
        ...
        public static bool SetForNewGame(ref SpaceSector __instance)
        {
            ...
            if (__instance.isCombatMode)
            {
                ...
                GS2.Log(string.Format("Initialize and generate space enemy complete, time cost = {0:0.00} ms",
                    duration * 1000.0));
            }
        ...
        public static void GameData_Import(GameData __instance)
        {
            for (int i = 0; i < __instance.factoryCount; i++)
            {
                ...
                for (int monitorId = 1; monitorId < factory.cargoTraffic.monitorCursor; monitorId++)
                {
                    ...
                    {
                        if (factory.cargoTraffic.monitorPool[monitorId].speakerId >=
                            factory.digitalSystem.speakerCursor)
                        {
                            ...
                            GS2.Warn(
                                $"{factory.planet.displayName}: Remove monitor {monitorId} for speakerId {speakerId} is out of bound");
