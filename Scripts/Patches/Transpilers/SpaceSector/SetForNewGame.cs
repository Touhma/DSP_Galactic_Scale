    ...
    public partial class PatchOnSpaceSector
    {
            ...
            IEnumerable<CodeInstruction> instructions)
        {
            var codeMatcher = new CodeMatcher(instructions)
                ...
                {
                    GS3.LogTranspilerError(instructions, "TranspileSpaceSectorSetForNewGameAstroCount failed.");
  
                    return instructions;
                }
                if (codeMatcher.IsValid)
                {
                    GS3.Log("TranspileSpaceSectorSetForNewGameAstroCount succeeded.");
                }
        ...
        private static int CalcAstroCapacity()
        {
            ...
            GS3.Log($"Set AstroCapacity to {cap} with {GameMain.spaceSector.galaxy.starCount} stars");
        return cap;
        }
    }
}