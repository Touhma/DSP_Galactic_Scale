    ...
    public class PatchOnGameLoader
    {
        ...
        public static bool FixedUpdate(ref GameLoader __instance)
        {
            ...
            if (__instance.frame >= 7)
            {
                ...
                counter++;
                if (planet != null && counter % 100 == 0)
                {
                    GS2.DevLog($"FRAME 7 ({counter}) LocalStar:{GameMain.localStar?.name} Local Planet:{planet?.name} LocalPlanet Loaded:{planet.loaded} LocalPlanet FactoryLoaded:{planet.factoryLoaded}");

                }
                if (planet != null && counter % 500 == 0)
                {
                    GS2.DevLog($"FRAME 7 ({counter}) LocalStar:{GameMain.localStar?.name} Local Planet:{planet?.name} LocalPlanet Loaded:{planet.loaded} LocalPlanet FactoryLoaded:{planet.factoryLoaded} ATTEMPTING TO RELOAD");
                    planet.Load();
                }
