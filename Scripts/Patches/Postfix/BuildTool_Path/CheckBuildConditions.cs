...
namespace GalacticScale.Patches
{
    public partial class PatchOnBuildTool_Path
    {
        ...
        {
            // GS3.Warn(__instance.buildPreviews[0].condition.ToString());
            ...
            if (entity.id == 0 && entity2.id == 0) return __result;
            ...
            //GS3.Log($"{itemProto?.prefabDesc?.isStation} | {itemProto2?.prefabDesc?.isStation}");
            //GS3.Log($"{itemProto2?.Name}");
            ...
            if (requiresPatch)
            {
                //GS3.Log($"{preview?.condition} | {preview2?.condition}");
