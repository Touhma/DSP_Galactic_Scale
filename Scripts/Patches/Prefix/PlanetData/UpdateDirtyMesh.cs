...
namespace GalacticScale.Patches
{
    public partial class PatchOnPlanetData
    {
        ...
        [HarmonyTranspiler]
        [HarmonyPatch(typeof(PlanetData), nameof(PlanetData.UpdateDirtyMesh))]
        public static IEnumerable<CodeInstruction> UpdateDirtyMeshTranspiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (var i = 0; i < codes.Count; i++)
                    ...
                    // We stop checking this 1 early because we operate on both the current AND following line when we do anything
                {
                    ...
                    if (codes[i + 1].LoadsField(typeof(PlanetData).GetField(nameof(PlanetData.scale))))
                    {
                ...
                else if (codes[i].Calls(typeof(PlanetRawData).GetMethod(nameof(PlanetRawData.GetModPlane))))
                    // This condition finds calls to PlanetRawData.GetModPlane (which returns a short)
                {
                    ...
                    codes[i] = new CodeInstruction(OpCodes.Call, typeof(Utils).GetMethod(nameof(Utils.GetModPlaneInt)));
                }
