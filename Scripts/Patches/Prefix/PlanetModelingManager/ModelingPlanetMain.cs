...
namespace GalacticScale.Patches
{
    ...
    {
        [HarmonyPrefix]
        [HarmonyPatch(typeof(PlanetModelingManager), nameof(PlanetModelingManager.ModelingPlanetMain))]
        ...
        public static IEnumerable<CodeInstruction> ModelingPlanetMainTranspiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (var instructionCounter = 0; instructionCounter < instructionList.Count; instructionCounter++)
                if (instructionList[instructionCounter].Calls(typeof(PlanetData).GetProperty(nameof(PlanetData.realRadius)).GetGetMethod()))
                {
                ...
                else if (instructionList[instructionCounter].Calls(typeof(PlanetRawData).GetMethod(nameof(PlanetRawData.GetModPlane))))
                {
                    //GS3.Log("Found GetModPlane callvirt. Replacing with GetModPlaneInt call.");
                    instructionList[instructionCounter] = new CodeInstruction(OpCodes.Call, typeof(Utils).GetMethod(nameof(Utils.GetModPlaneInt)));
                }
