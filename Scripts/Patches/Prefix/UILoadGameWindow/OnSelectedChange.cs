...
namespace GalacticScale.Patches
{
    public partial class PatchOnUILoadGameWindow
    {
        ...
        public static bool OnSelectedChange(UILoadGameWindow __instance)
        {
            ...
            GS3.Warn($"Header Version:{gameSaveHeader?.headerVersion}");
            if (gameSaveHeader != null)
            {
                ...
                var num = (int)(gameSaveHeader.gameTick % 60L);
                var num2 = (int)(gameSaveHeader.gameTick / 60L);
                var num3 = num2 / 60;
                var num4 = num3 / 60;
                num2 %= 60;
                num3 %= 60;
                __instance.prop2Text.text = string.Format("{0:00}:{1:00}:{2:00} <size=11>{3:00}</size>", num4, num3, num2, num);
                ...
                {
                    var productIds = PropertySystem.productIds;
                    var num5 = 0;
                    for (var i = 0; i < productIds.Length; i++)
                    {
                        var itemProduction = clusterPropertyData.GetItemProduction(productIds[i]);
                        num5 += itemProduction;
