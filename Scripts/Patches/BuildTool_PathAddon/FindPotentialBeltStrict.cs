...
namespace GalacticScale
{
	...
	Array.Clear(__instance.potentialBeltObjIdArray[bpIndex], 0, __instance.potentialBeltObjIdArray[bpIndex].Length);
	...
	{
		Vector3 lineStart = buildPreview.lpos + buildPreview.lrot * (addonAreaColPoses[i].position + addonAreaColPoses[i].forward * addonAreaSize[i].z * 2.5f);
		Vector3 lineEnd = buildPreview.lpos + buildPreview.lrot * (addonAreaColPoses[i].position - addonAreaColPoses[i].forward * addonAreaSize[i].z * 2.5f);
		...
		if (num > 0)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				if (prefabDesc != null && prefabDesc.isBelt && Maths.DistancePointLine(objectPose.position, lineStart, lineEnd) <= 0.3f)
				{
					if (__instance.potentialBeltCursorArray[bpIndex] >= __instance.potentialBeltObjIdArray[bpIndex].Length)
					{
						int[] array = __instance.potentialBeltObjIdArray[bpIndex];
						__instance.potentialBeltObjIdArray[bpIndex] = new int[__instance.potentialBeltCursorArray[bpIndex] * 2];
						if (array != null)
						{
							Array.Copy(array, __instance.potentialBeltObjIdArray[bpIndex], __instance.potentialBeltCursorArray[bpIndex]);
						}
					}
					__instance.potentialBeltObjIdArray[bpIndex][__instance.potentialBeltCursorArray[bpIndex]] = ((Mathf.Abs(num2) << 4) + i) * (int)Mathf.Sign((float)num2);
